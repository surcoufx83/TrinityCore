...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (target)
    {
        ...
        if (non_caster_target)
        {
            ...
            if (!m_IsTriggeredSpell && VMAP::VMapFactory::checkSpellForLoS(m_spellInfo->Id) && !(m_spellInfo->AttributesCu & SPELL_ATTR0_CU_IGNORE_LOS) && !m_caster->IsWithinLOSInMap(target))
