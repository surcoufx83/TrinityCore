...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (target)
    {
        ...
        if (non_caster_target)
        {
            ...
            if (!(_triggeredCastFlags & TRIGGERED_IGNORE_LOS) && !(m_spellInfo->AttributesCu & SPELL_ATTR0_CU_IGNORE_LOS) && VMAP::VMapFactory::checkSpellForLoS(m_spellInfo->Id) && !m_caster->IsWithinLOSInMap(target))
